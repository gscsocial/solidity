	...
	try
	{
		...
		while (true)
		{
			...
			if (currentTokenValue == Token::RBrace)
				...
				currentTokenValue == Token::SubEther ||
...
ASTPointer<ASTNode> Parser::parseFunctionDefinition()
{
	...
	{
		solAssert(kind == Token::Constructor || kind == Token::Fallback || kind == Token::SubEther, "");
		m_scanner->next();
